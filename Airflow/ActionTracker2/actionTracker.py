...
def createPlot(date, imagePath1):
    ...
    mpl.rcParams['axes.unicode_minus'] = False  # 正负号
    ...
    ax1.set_ylabel(u'总数(million)', fontsize=16, color=titleColor)
    ax2 = ax1.twinx()
    ax2.plot(x, y2, color=lineColor, linewidth=2)  # 折线图
    ax2.set_ylabel(u'漏失率(%)', fontsize=16, color=titleColor)  # 设置标签
    ...
    ax3.set_ylabel(u'漏失数量(Last 15 Days)', fontsize=16, color=titleColor)
    ax4 = ax3.twinx()
    ...
    ax5.set_ylabel(u'漏失数量(Last Day)', fontsize=16, color=titleColor)
    ax6 = ax5.twinx()
