...
def insUmpToClk(database, tableNm, minutes):
    querySql3 = f"""select id from traceparafailed where createtime>now()-interval '{minutes / 60 * -4} hour' ;"""  # 查询traceparafailed数据
    ...
    querySql2 = f"""select id from {tableNm} where createtime>now()-interval '{minutes / 60 * -4} hour';"""  # 查询分表已导入数据
    if len(ids) > 0:
