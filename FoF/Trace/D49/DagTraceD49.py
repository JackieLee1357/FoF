...
from Trace.TraceBin.traceBin import RunTraceBin
from Trace.traceAutoDel import RunAutoDel
...
def runTraceBin(**kwargs):
    RunTraceBin(project)


def runAutoDel(**kwargs):
    RunAutoDel(project)
...
t4 = PythonOperator(
    task_id=f'{project}RunTraceBin',
    provide_context=True,
    python_callable=runTraceBin,
    dag=dag)

t5 = PythonOperator(
    task_id=f'{project}RunAutoDel',
    provide_context=True,
    python_callable=runAutoDel,
    dag=dag)
...
OneFailed.set_upstream([t1, t2, t3, t4, t5])
