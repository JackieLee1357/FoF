...
import datetime
import pandas as pd
import TraceMQ.D49.dfToClk as dk

pd.set_option('display.max_columns', None)  # 显示所有列
pd.set_option('display.max_rows', None)  # 显示所有行


def insUmpToClk(pjt, db, startTime):
    querySql = f"""SELECT id, results, project, process, createtime
            FROM JGPWMD49.TraceMQResult prewhere createtime>toDateTime('{startTime}');"""  # 查询一小时内的D49数据
...
def RunTraceResultD49():
    ...
    try:
        insUmpToClk(project, table, startTime)
    except Exception as e:
