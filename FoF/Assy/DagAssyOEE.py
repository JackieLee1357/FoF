...
default_args = {
    ...
    "start_date": datetime.datetime.now().replace(minute=2, second=40, microsecond=0) + datetime.timedelta(hours=-2),
    # 开始时间
...
dag = DAG("AssyOEE", default_args=default_args, schedule_interval=datetime.timedelta(hours=1))
t1 = BashOperator(
...
t1Failed = EmailOperator(
    ...
    task_id="Run_AssyOEE_Failed",
    to=["yuan_li5928@jabil.com", "zhuang_hu@jabil.com", "glenn_yang@jabil.com"],
    subject="Run AssyOEE Failed",
    html_content='<h3>Run AssyOEE Failed, Please Check!</h3>')
