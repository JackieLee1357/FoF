...
class MyThread(threading.Thread):
    def __init__(self, database, tableName, data):
    ...
    def run(self):
        ...
        insertToClk(self.database, self.tableName, self.data)
...
def insertToClk2(database, tableName, a):
    data_list = a.to_dict("records")
    n = len(data_list)
    logging.info(f"insert data to {tableName}... total: {n}.")
...
def openThread(database, tableName, data):
    # 创建新线程
    thread1 = MyThread(database, tableName, data)
    # 开启新线程
    thread1.start()
    # History.start()
    thread1.join()
...
def traceMqConsumer(topic, tableName):
    ...
    for msg in consumer:
        ...
        try:
            ...
            del data["project"]
            openThread(database, tableName, data)
            if i % 10000 == 0:
