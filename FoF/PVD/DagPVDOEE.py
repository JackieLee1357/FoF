...
default_args = {
    ...
    "start_date": datetime.datetime.now().replace(minute=3, second=30, microsecond=0) + datetime.timedelta(hours=-2),
    # 开始时间
...
dag = DAG("PVDOEE", default_args=default_args, schedule_interval=datetime.timedelta(hours=1))
t1 = BashOperator(
...
t1Failed = EmailOperator(
    ...
    task_id="Run_PVDOEE_Failed",
    to=["yuan_li5928@jabil.com", "zhuang_hu@jabil.com"],
    subject="Run PVDOEE Failed",
    html_content='<h3>Run PVDOEE Failed, Please Check!</h3>')
