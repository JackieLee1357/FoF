...
def dataDuplicated(frame):
    frame.iloc[0, 4] = pandas.to_datetime(eventTime)  # 第一行时间改为开始时间
    frame['lastsTime'] = 0
    for i in range(len(frame)):
        if i == len(frame) - 1:
            frame.loc[i, 'lastsTime'] = (pandas.to_datetime(endTime) - frame.iloc[i, 4]).seconds  # 计算最后一行时间
        else:
            frame.loc[i, 'lastsTime'] = (frame.iloc[i + 1, 4] - frame.iloc[i, 4]).seconds
    # print('数据清洗完毕')
    return frame
...
def statusCalculate(fms):
    ...
    dt = pandas.DataFrame([[emt, recipeNumber, eventDate, status1, status2, status3, status4, status5, status6, status7]])
    dt.columns = ["emt", "recipeNumber", "eventTime", "status1", "status2", "status3", "status4", "status5", "status6", "status7"]  # 修改列名
...
def errorCodeCalculate(fms):
    columns = ['emt', 'RecipeNumber', 'status', 'ErrorCode', 'eventTime', 'lastTime']
    fms.columns = columns
...
--             ORDER BY CT DESC"""
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        # print(machineList)
        sql = f"""SELECT distinct [EMT],[RecipeNumber], [Status],[ErrorCode],[EventTime]
        ...
        """
        columns = ['emt', 'RecipeNumber', 'status', 'errorcode', 'eventtime']
        ...
        for j in range(len(machineList)):  # 遍历机台号计算DOWNTIME
            ...
            if (emtData0 is None) or (len(emtData0) == 0):
                continue
            recipeNumber = emtData0.iloc[0, 1]
