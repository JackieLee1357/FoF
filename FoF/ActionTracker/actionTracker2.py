...
def readDataSource(pathName, dateDay, dateNow0, project0):
    # 读取数据源excel，进行数据处理，并返回DataFrame
    sheetKeys = pd.read_excel(pathName, sheet_name=None)  # 读取excel
    pd.set_option('display.max_columns', None)  # 显示所有列
    pd.set_option('display.max_rows', None)  # 显示所有行
    sheetData = pd.read_excel(pathName, sheet_name=list(sheetKeys).index('数据'))
    ...
    index = list(sheetData.values[1]).index(dateDay)  # 当前日期的index
    print('--------')
    ...
    for i in range(len(sheetData)):
        if sheetData.loc[i, 0] == '制程':
            ...
            break
    sheetData.loc[0:, '日期'] = dateNow0
    sheetData.loc[0:, 'project'] = project0
    sheet_data_table_name = ['制程', '站别', '被卡控站点', '漏失目标', 'dri', '呈现工站', '总数', '漏失数量', '漏失率',
                          '问题点描述', '问题类型', '日期', 'project']
    sheetData.columns = sheet_data_table_name
    print('源数据读取完毕')
    print('------------')
    return sheetData
...
def dataSourceSplit(sheetData, project0):
    ...
    traceMissingTable["item_id"] = [project0 + str(maxId + i + 1) for i in range(len(dfList))]
    ...
    try:
        pd.to_datetime(traceMissingTable.loc[0:, '日期'], format='%Y-%m-%d')
        pd.to_datetime(traceMissingTable.loc[0:, 'returntime'])
    except Exception as e:
        print(e)
        print("excel源文件日期格式错误" + str(e))
        sys.exit(1)
    try:
        pd.to_numeric(traceMissingTable.loc[0:, '漏失数量'])
        pd.to_numeric(traceMissingTable.loc[0:, '漏失率'])
    except Exception as e:
        print(e)
        print("excel源文件数字格式错误，请检查站点名称是否错误~" + str(e))
        sys.exit(1)
    ...
    pd.to_datetime(FACATable.loc[0:, 'eventtime'])
    # pd.to_numeric(FACATable.loc[0:, 'id'])
    print('数据处理完成')
    print('------------')
...
def getMailData(project0, dateNow0, imagePath0):
    # pd.DataFrame(dataFrame).to_excel('C:\ActionTracker\data1.xlsx', sheet_name='summery', index=False,
    # header=True)
    tableColumns = ['日期', '被卡控站点', '呈现站点', '拦截数量', '拦截率', 'FA', 'CA', 'DRI']
    sqlData = f""" SELECT DISTINCT top (30) 
                         ...
                         WHERE ((A.日期 = '{dateNow0}' and b.eventtime = '{dateNow0}') 
                         or (A.returntime='1900-01-01 00:00:00.000' or A.returntime is null ))
                         and A.item_id like '{project0}%' and B.item_id like '{project0}%'
                         ORDER BY A.日期 DESC """  # 选择当天数据和之前未回复的数据,按日期倒序排列
    mailInfoTable0 = pd.DataFrame(conSqlS(sqlData))  # 从数据库获取需回复FACA信息
    mailInfoTable0.columns = tableColumns
    mailInfoTable0['日期'] = mailInfoTable0['日期'].astype(str)  # 强制转换数据类型
    for i in range(len(mailInfoTable0.loc[0:, '拦截率'])):
        mailInfoTable0.loc[i, '漏失率'] = '{:.2%}'.format(float(mailInfoTable0.loc[i, '拦截率']))  # 格式化输出%数据
    ...
    mailInfoTable1 = mailInfoTable0[['日期', '呈现站点', '拦截数量', '拦截率']]
    mailInfoTable1 = mailInfoTable1.style.set_table_styles(styles).applymap(changeColor,
                                                                            subset=['拦截率']).hide_index().render()
    mailInfoTable1 = changeStyle(mailInfoTable1, dateNow0, imagePath0, project0)  # 各站点总的拦截率
    # TODO
    mailInfoTable0 = mailInfoTable0.style.set_table_styles(styles).applymap(changeColor,
                                                                            subset=['拦截率']).hide_index().render()
    mailInfoTable0 = changeStyle(mailInfoTable0, dateNow0, imagePath0, project0)  # 各站点分楼栋拦截率
    return mailInfoTable1
...
def changeStyle(mailInform, dateNow1, imagePath0, project0):
    # 未达标邮件~
    with open(imagePath0, "rb") as f:  # 图片转为base64格式
    ...
    """.format(dataFrame=mailInform, project=project0, sh_sys=str(dateNow1)[0:10], imageBase64=imageBase64)
...
def changeStyle2(dateNow1, project0):
    # 达标邮件~
    head = \
        """
        <head>
    ...
    """.format(project=project0, sh_sys=str(dateNow1)[0:10])
    mailText = "<html>" + head + body + "</html>"
    print('mail格式处理完毕')
    print('------------')
    return mailText
...
def changeColor(val):
    ...
    """
    try:
        if float(val) > 0.05:  # 报警规则 val[:-1]) > 0.05
            return 'color:red'
        else:
            return 'color:black'
    except Exception as e:
        print('changeColor Error:' + str(e))
        return 'color:black'


def conPlotData(date, project0):  # 链接数据库，查询已处理的时段
...
def createPlot(date, imagePath1, project0):
    dfs = conPlotData(date, project0)
    ...
    for a, b in zip(x, y1):  # 设置数字标签**
        ax1.text(a, b, '{:,}'.format(b), ha='center', va='bottom', fontsize=13)  # a, b 为标签位置,数字中添加千分位',''%.0f' % b
    ...
    xticks3 = ax3.get_xticks()  # 获取x标签的坐标值
    for a, b in zip(xticks3, y):  # 设置数字标签**
        # ax3.text(a, b, '%.0f' % b, ha='center', va='bottom', fontsize=13)  # a, b 为标签位置
        ax3.text(a, b, '{:,}'.format(b), ha='center', va='bottom', fontsize=13)  # a, b 为标签位置,数字中添加千分位',''%.0f' % b
    for a, b in zip(xticks3, y2):  # 设置数字标签**
    ...
    for tick in ax5.get_xticklabels():  # 刻度标签旋转
        tick.set_rotation(7.5)
    xticks5 = ax5.get_xticks()  # 获取x标签的坐标值
    for a, b in zip(xticks5, y):  # 设置数字标签**
        # ax5.text(a, b, '%.0f' % b, ha='center', va='bottom', fontsize=13)  # a, b 为标签位置
        ax5.text(a, b, '{:,}'.format(b), ha='center', va='bottom', fontsize=13)  # a, b 为标签位置,数字中添加千分位',''%.0f' % b
    for a, b in zip(xticks5, y2):  # 设置数字标签**
...
class SendMail(object):
    def __init__(self, content=None,
                 image=None, file=None, recv=None, cc_email0=None, project0=None):
        """
        ...
        self.cc_email = cc_email0  # cc人员
        self.title = f'{project0}专案Trace漏失报告'  # 邮件标题
...
if __name__ == '__main__':
    ...
    iniPath = [iniPath + r"/actionTracker.ini", iniPath + r"/actionTracker2.ini", iniPath + r"/actionTracker3.ini"]
    print(iniPath)
        ...
        try:
            ...
            if (not isRun) and (not dataIsNone):  # 数据没有运行，且数据不为空时，存入数据库。防止重复存入数据库
                ...
                if datetime.datetime.now().hour != 8:  # 防止重复运行重复存入数据库重复发邮件
                    ...
                    continue
                insertIntoSql(dataList, SQLTableName)  # 3.数据存入PGSQL
                mailInfoTable = changeStyle2(dateNow, project)
                m = SendMail(content=mailInfoTable, recv=people_email, cc_email0=cc_email, project0=project)
                m.send_mail()  # 7. 发Trace无漏失邮件
                continue

            createPlot(dateNow, imagePath, project)  # 从数据库生成图片
            ...
            if isRun & (reMail != '1'):  # 防止重复运行重复存入数据库重复发邮件
                print('当天邮件已发送~')
                continue
            m = SendMail(content=mailInfo, recv=people_email, cc_email0=cc_email, project0=project)
