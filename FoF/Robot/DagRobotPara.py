...
default_args = {
    ...
    "start_date": datetime.datetime.now().replace(minute=6, second=10, microsecond=0) + datetime.timedelta(hours=-2),
    # 开始时间
...
dag = DAG("RobotPara", default_args=default_args, schedule_interval=datetime.timedelta(hours=1))
t1 = BashOperator(
...
t1Failed = EmailOperator(
    ...
    task_id="Run_RobotPara_Failed",
    to=["yuan_li5928@jabil.com", "zhuang_hu@jabil.com", "yujie_wu@jabil.com", "huiwen_xu@jabil.com"],
    subject="Run RobotPara Failed",
    html_content='<h3>Run RobotPara Failed, Please Check!</h3>')
