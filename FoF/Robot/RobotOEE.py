...
if __name__ == '__main__':
    ...
    for i in range(endHour - 1, beginHour - 1, -1):  # 按小时倒序循环
        ...
        sql = f"""WITH ONE AS(
            ...
            AND Station like 'WXI_Metal_%' and Type='ROBOT'
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        if eventDate in times:
            print('该时段数据已处理')
            continue
        sql = f"""SELECT distinct [EMT] FROM [SMDP].[dbo].[viewSMEAPI_MachineStatus] WHERE Station like 'WXI_Metal_%' and Type='ROBOT'
