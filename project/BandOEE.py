...
def duplicated(frame):
    ...
    if len(frame) != 0:
        if frame.loc[len(frame)-1][1] == '3':
            print('最后一行状态为3')
            frame.loc[len(frame)] = [frame.loc[len(frame)-1][0],'1',frame.loc[len(frame)-1][2],pandas.to_datetime(endTime)]    #最后面新增一行数据
    print('数据清洗完毕')
    return frame
...
def conPGSQL(tableName):             #链接PG数据库，查询已处理的时段
    ...
    cursor.execute(f'select distinct eventtime from {tableName};')
    row = cursor.fetchall()
...
if __name__ == '__main__':
    ...
    for i in range(endHour - 1, beginHour - 1, -1):              #按小时倒序循环
        ...
        minTime = i
        j = -maxTime
        eventDate = str((datetime.datetime.now() + datetime.timedelta(hours=minTime)).strftime(f"%Y-%m-%d %H:00:00"))  #发生时段
    ...
    for i in range(endHour-1, beginHour-1, -1):
        ...
        maxHour = i+1
        eventDate = str((datetime.datetime.now() + datetime.timedelta(hours=minHour)).strftime(f"%Y-%m-%d %H:00:00"))   #发生的时段
        endTime = str((datetime.datetime.now() + datetime.timedelta(hours=maxHour)).strftime(f"%Y-%m-%d %H:00:00"))     #时段结束时间
        print('开始时间' + eventDate)
        if eventDate in times:
            print('该时段数据已处理')
            continue
