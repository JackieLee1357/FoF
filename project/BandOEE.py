...
if __name__ == '__main__':
    ...
    for i in range(endHour - 1, beginHour - 1, -1):  # 按小时倒序循环
        ...
        sql = f"""WITH ONE AS(
            ...
            AND EMT IN (select distinct EMT from viewMachineList WHERE Station like 'WXI_Metal_%' and Type='CNC')
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        sql = f"""SELECT distinct [EMT] FROM [SMDP].[dbo].[viewSMEAPI_MachineStatusMonth] WHERE EMT IN (select distinct EMT from viewMachineList WHERE Station like 'WXI_Metal_%' and Type='CNC')
