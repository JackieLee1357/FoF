...
def conPGSQL(tableName):             #链接PG数据库，查询已处理的时段
    ...
    cursor.execute(f'select distinct downdate from {tableName};')
    row = cursor.fetchall()
...
if __name__ == '__main__':
    ...
    tableName = 'bandoee_downtimedaily'
    date = str((datetime.datetime.now() + datetime.timedelta(days=endDay)).strftime("%Y/%m/%d"))[:10]     #截至日期  2021/07/24
    ...
    while date_compare(date,date2,"%Y/%m/%d"):
        ...
        if dateTmp in times:
            ...
            continue
        url = f"http://cnwxim0kaba01.corp.jabil.org:8081/pis_m_api/api/OEE_DownTimeModule/GetQueryDailyDownTimeDetail?Plant=WUXI_M&BG=WUXI%20Metal&ProjectName&Line&Station&ProductDate={str(date)}"
