...
def read_excel(sheetname, columns, excelname):
    ...
    """
    excel_data = pandas.read_excel('//Cnwxig0nsifsn01\wxi mes\Metal/5.Tech Support/1/WATER.xlsx', sheet_name=sheetname,
...
if __name__ == '__main__':
    try:
        ...
        if last_time > datetime.datetime.now():
            last_time = datetime.datetime.now().replace(minute=0, second=0, microsecond=0) + datetime.timedelta(hours=-1)
