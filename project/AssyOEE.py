...
def statusCalculate(fms):
    ...
    for i in range(len(downtime)):
        ...
        elif downtime.iloc[i, 0] == 8:
            status8 = downtime.loc[i, 'lastsTime']
        elif downtime.iloc[i, 0] == 9:
            status9 = downtime.loc[i, 'lastsTime']
    dt = pandas.DataFrame(
        [[dsn, eventDate, status1, status2, status3, status4, status5, status6, status7, status8, status9]])
    dt.columns = ["dsn", "eventTime", "status1", "status2", "status3", "status4", "status5", "status6", "status7",
                  "status8", "status9"]  # 修改列名
...
def errorCodeCalculate(fms):
    columns = ['dsn', 'status', 'ErrorCode', 'eventTime', 'lastTime']
    fms.columns = columns
    print(fms)
    ...
    errorCodeTime['dsn'] = dsn
    errorCodeTime['status'] = 3
    errorCodeTime['eventTime'] = eventDate
    errorCodeTime.columns = ['ErrorCode', 'lastTime', 'dsn', 'status', 'eventTime']
    errorCodeTime = errorCodeTime[['dsn', 'status', 'ErrorCode', 'eventTime', 'lastTime']]
    print(errorCodeTime)
    print("===========================")
...
if __name__ == '__main__':
    ...
    for i in range(endHour - 1, beginHour - 1, -1):  # 按小时倒序循环
        ...
        if eventDate in times:
            print('该时段数据已处理')
            continue
        columns = ['dsn', 'output', 'eventtime', 'hourindex', 'ct']
        sql = f"""WITH ONE AS(
            SELECT DSn
            ,DATEDIFF(MINUTE,'{eventDate}',EndTime)/60+1 hourIndex
            ,COUNT(DISTINCT SerialNumber) totalCount
            ,avg(CONVERT
            (decimal(16,2),case isnull(ActualCT,'0') when '0' then '0' when '' then '1' else ActualCT end))  as CT
            FROM [OEE].[dbo].[viewSMEAPI_WIPData]
            where EndTime>='{eventDate}'
            and EndTime<'{endTime}'
            AND DSn IN (select distinct DSn from [OEE].[dbo].[API_RegisteredMachine] WHERE Factory like 'WUXI-Metal-%')
            group by DSn,DATEDIFF(MINUTE,'{eventDate}',EndTime)/60)
            SELECT DSn            
            ,CASE WHEN totalCount=0 THEN 0  
            else totalCount - 1 END AS output
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        if eventDate in times:
            print('该时段数据已处理')
            continue
        sql = f"""SELECT distinct dsn FROM [OEE].[dbo].[viewSMEAPI_MachineStatus] WHERE dsn IN 
        (select distinct DSn from [OEE].[dbo].[API_RegisteredMachine] WHERE Factory like 'WUXI-Metal-%')
        AND EventTime > '{eventDate}'
        AND EventTime< '{endTime}'
        GROUP BY dsn"""  # 查询机台号EMT
        machineList = getDataFromSqlS(sql)  # 获取机台号
        sql = f"""SELECT distinct [dsn],[Status],[TotalNum],[EventTime]
              FROM [OEE].[dbo].[viewSMEAPI_MachineStatus]
              WHERE EventTime > '{beginTime}'
              AND EventTime < '{endTime}' and dsn IN 
              (select distinct DSn from [OEE].[dbo].[API_RegisteredMachine] WHERE Factory like 'WUXI-Metal-%')
        """
        columns = ['dsn', 'status', 'totalnum', 'eventtime']
        rowData = getDataFromSqlS(sql)  # 获取两小时的数据
        rowData.columns = columns
        print(f"{eventDate}数据分析中")
        for j in range(len(machineList)):  # 遍历机台号计算DOWNTIME
            dsn = machineList.loc[j][0]
            emtData0 = rowData.loc[rowData[rowData['dsn'] == dsn].index]  # 筛选机台号数据
            ...
            print(f"第{j}个EMT:{dsn}，EvenTime:{eventDate}")
            print(emtData)
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        (select distinct DSn from [OEE].[dbo].[API_RegisteredMachine] WHERE Factory like 'WUXI-Metal-%')
                 ...
                 GROUP BY dsn"""  # 查询机台号EMT
        machineList = getDataFromSqlS(sql)  # 获取机台号
        sql = f"""SELECT distinct [dsn],[Status],[ErrorCode],[EventTime]
                      FROM [OEE].[dbo].[viewSMEAPI_MachineStatus]
                      WHERE EventTime > '{beginTime}'
                      AND EventTime < '{endTime}' and dsn IN 
                      (select distinct DSn from [OEE].[dbo].[API_RegisteredMachine] WHERE Factory like 'WUXI-Metal-%')
                """
        columns = ['dsn', 'status', 'errorcode', 'eventtime']
        rowData = getDataFromSqlS(sql)  # 获取两小时的数据
        rowData.columns = columns
        # print(f"{eventDate}数据分析中")
        for j in range(len(machineList)):  # 遍历机台号计算DOWNTIME
            ...
            # print("----------------------")
            print(f"第{j}dsn:{dsn}，EvenTime:{eventDate}")
            # print(emtData)
            if (emtData is None) or (len(emtData) == 0):
                continue
            fms = dataDuplicated(emtData)  # 计算每条记录的持续时间
