...
def change_style(fort_can):
    ...
    #         <!--标题部分的信息-->
    #         <h1 align="center">JGP无锡废水超标报警 </h1>
    #     </div>
    #
    #     <hr>
    #
    ...
    #         <!--正文内容-->
    #         <h2>Dear leader:</h2>
    #         <h5>以下为{starttime}-{endtime}废水超标数据详情，请查阅！<p>完整报告请至http://cnwgpm0pbi01/Reports/powerbi/WX_MetalSME/official/JGP%E5%BA%9F%E6%B0%B4%E7%9B%91%E6%B5%8B%E6%8A%A5%E5%91%8A</h5>
    #     <div>
    #         <h4>{fort}</h4>
    #     </div>
    #     <hr>
    ...
    # </div>
    # </body>
    # """.format(fort=fort,starttime=starttime,endtime=last_time)
    # #修改html,制作email样式模板.
...
def Godsaycolor(idx_YES, html_text, idx_NO):

    # td_text = re.findall("<tr>{x}</tr>".format(x=idx_row), html_text)
    # text = td_text[(idx_row) * int(6) + idx_col]
    # print(text)

    html_text_fin = html_text.replace("<th>{x}</th>".format(x=idx_YES),'<tr bgcolor="#ff0000">')
    html_text_fin = html_text_fin.replace("<th>{x}</th>".format(x=idx_NO),'<tr bgcolor="#E6E6FA">')
    html_text_fin = html_text_fin.replace('<th>s</th>','')
    html_text_fin = html_text_fin.replace('<th>t</th>','<th>项目</th>')
    html_text_fin = html_text_fin.replace('<th>m</th>','<th>地点</th>')
    html_text_fin = html_text_fin.replace('<th>d</th>','<th>值</th>')
    html_text_fin = html_text_fin.replace('<th>k</th>','<th>上限</th>')
...
def change_style_df(df):
    ...
    dataFrame=dataFrame.rename(columns={'DataTime': 't', 'ParamName': 'm', 'MNName': 'd', 'DataValue': 'k', 'BZlevelUP': 's','result':'x'})
    dataFrame=dataFrame.reset_index(drop=True)
    dataFrame.index=dataFrame.index+1
    a = dataFrame[(dataFrame['x']=='YES')].index.tolist()
    b = dataFrame[(dataFrame['x']!='YES')].index.tolist()
    dataFrame.drop(columns=['x'],inplace=True)
    if len(a)!=0:
        dataFrame_html=dataFrame.to_html()
        print(dataFrame_html)

        for i in a:
            for x in b:
                dataFrame_html = Godsaycolor(idx_NO=x, html_text=dataFrame_html, idx_YES=i)
    else:
        dataFrame_html=dataFrame.to_html()

    # fmt = lambda x: '{0:.2f}'.format(x) if pandas.notnull(x) else 'Ted'
    # fme= lambda x: 'color:red'
    # dataFrame=dataFrame.to_html(float_format=fmt,formatters={'地点':lambda x: 'color:"red"' + x + '</font>'})
    # dataFrame=dataFrame.replace('<tr>', '<tr align="center">')
    ...
    #         ]#字典的方式给下面的style传值
    # dataFrame = dataFrame.style.applymap(change_color,subset=pandas.IndexSlice[float(a[0]):float(a[len(a) - 1]), ['s', 'x', 't', 'm', 'd', 'k']]).hide_index().render()

#            set_table_styles([{'selector': 'td','props': [('font-family', 'arial')]}]). \
#            set_table_styles([{'selector': 'td:hover','props': [('border', '1px solid black')]}]). \
    ...
    body = \
        ...
        <div class="content">
            <!--正文内容-->
            <h2>Dear leader:</h2>
            <h5>以下为{starttime_html}-{endtime_html}废水数据详情，请查阅！<p>完整报告请至http://cnwgpm0pbi01/Reports/powerbi/WX_MetalSME/official/JGP%E5%BA%9F%E6%B0%B4%E7%9B%91%E6%B5%8B%E6%8A%A5%E5%91%8A</h5>
        <div>
            <h4>{dataFrame}</h4>
        </div>
        <hr>
    ...
    """.format(dataFrame=dataFrame_html,starttime_html=endtime_oftotal,endtime_html=starttime)
    #修改html,制作email样式模板.
...
def read_excel(sheetname,columns):
    """
    ...
    """
    excel_data=pandas.read_excel('C:\\water\\WATER.xlsx',sheet_name=sheetname, engine="openpyxl",header=0)
...
def change_style_excel(excel_one,time_html):
    ...
    body = \
        ...
        <div align="center" class="header">
            <!--标题部分的信息-->
            <h1 align="center">JGP无锡实验室数据详情</h1>
        </div>
        ...
        <div class="content">
            <!--正文内容-->
            <h2>Dear leader:</h2>
            <h5>以下{html_time}为实验室数据详情，请查阅！</h5>
        <div>
            <h3>{dataFrame}</h3>
        </div>
        <hr>
    ...
    """.format(dataFrame=excel_one,html_time=time_html)
    dataFrame = "<html>" + head + body + "</html>"
...
if __name__ == '__main__':
    ...
    html_excel_time_end=excel_time[11:19]

    #if html_excel_time_end==excel_time2:
    if html_excel_time_end==4396:
        data_colums = ['处理系统', '项目', '客户标准(min)', '客户标准(max)', excel_time]  # sheet1的表头
        note_colums=['处理系统',excel_time]
        data_excel=read_excel(sheetname=0,columns=data_colums)
        note_excel=read_excel(sheetname=1,columns=note_colums)
        ...
        overweight(guest_min=x['客户标准(min)'],guest_max=x['客户标准(max)'],time_new=x[excel_time],project=x['项目']),axis=1)
        ...
        data_excel_true = data_excel_true.groupby(['处理系统'], as_index=False).apply(lambda x: merge_group(excel_rersult=x['result']))
        # 对数据进行归一
