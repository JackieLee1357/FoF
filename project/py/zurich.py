...
def link_select_sql(con, start_time, next_time):
    ...
    SUBSTRING(bg,len(bg),1) as bgeeee,
    event,lastupdate,
    stationid,process FROM [Trace_D63].[dbo].[V_History_Temp] with (nolock)
    where  lastupdate >'{time_next_sql}' and lastupdate <='{time_start_sql}'
    and expr1 not in (SELECT expr1 from [Trace_D63].[dbo].[V_History_Temp] WITH(NOLOCK) where process = 'doe-in')
...
def insert_sql(data):
    ...
    output.seek(0)
    cursor.copy_from(output, 'zurich', null='')
    connection.commit()
    cursor.close()
    engine.dispose()
...
def del_sql(time_1, time_2, time_3, time_4):
    ...
    cursor = connection.cursor()
    # 删除今天0-6 27
    sql = '''delete  from zurich t where t.date='{a}' and
                  t.hour  in ('0','1','2','3','4','5')'''.format(a=time_1)
    print(sql)
    # 删除昨天一天 26
    sql2 = '''delete  from zurich t where t.date='{b}' '''.format(b=time_2)
    print(sql2)
    # 删除前天 25
    sql3 = '''delete  from zurich t where t.date='{c}' '''.format(c=time_3)
    print(sql3)
    # 删除大前天 6-24
    sql4 = '''delete  from zurich t where t.date='{d}' and
...
if __name__ == '__main__':
    ...
    config.read('c:\\TRACE_D63\\TRACE_D63.ini')
    ...
    config.write(open('c:\\TRACE_D63\\TRACE_D63.ini', "r+", encoding="utf-8"))
    print('时间已更新,运行下次')
