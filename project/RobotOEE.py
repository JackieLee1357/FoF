...
if __name__ == '__main__':
    ...
    for i in range(endHour - 1, beginHour - 1, -1):  # 按小时倒序循环
        ...
        sql = f"""WITH ONE AS(
            ...
            FROM [SMDP].[dbo].[View_API_CNCProductInfo] 
            where EventTime>='{eventDate}'
            and EventTime<'{endTime}'
            AND EMT IN (select distinct EMT from viewMachineList WHERE Station like 'WXI_Metal_%' and Type='ROBOT')
            ...
            else totalCount - 1 END END AS output
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        if eventDate in times:
            print('该时段数据已处理')
            continue
        sql = f"""SELECT distinct [EMT] FROM [SMDP].[dbo].[View_API_MachineStatus] WHERE EMT IN 
        (select distinct EMT from viewMachineList WHERE Station like 'WXI_Metal_%' and Type='ROBOT')
        ...
        sql = f"""SELECT distinct [EMT],[Status],[TotalNum],[EventTime]
              FROM [SMDP].[dbo].[View_API_MachineStatus]
    ...
    for i in range(endHour - 1, beginHour - 1, -1):
        ...
        if eventDate in times:
            print('该时段数据已处理')
            continue
        sql = f"""SELECT distinct [EMT] FROM [SMDP].[dbo].[View_API_MachineStatus] WHERE EMT IN (select distinct EMT from viewMachineList WHERE Station like 'WXI_Metal_%' and Type='ROBOT')
                 AND EventTime > '{eventDate}'
                 AND EventTime< '{endTime}' 
                 GROUP BY EMT"""  # 查询机台号EMT
        machineList = getDataFromSqlS(sql)  # 获取机台号
